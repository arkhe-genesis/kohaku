...
class Kyber768:
    ...
    def _unpack_secret_key(self, sk: bytes) -> Tuple[List[List[int]], bytes]:
        ...
        for i in range(self.k):
            ...
            for j in range(self.n):
                poly.append(int.from_bytes(s_data[i*poly_size + j*4 : i*poly_size + (j+1)*4], 'little', signed=True))
            s_ntt.append(poly)
        return s_ntt, z
    ...
    def _unpack_public_key(self, pk: bytes) -> Tuple[bytes, List[List[int]]]:
        ...
        for i in range(self.k):
            poly = []
            for j in range(self.n):
                poly.append(int.from_bytes(t_data[i*poly_size + j*4 : i*poly_size + (j+1)*4], 'little', signed=True))
            t.append(poly)
        return rho, t
    ...
    def encapsulate(self, pk: bytes) -> Tuple[bytes, bytes]:
        ...
        v = [(v[i] + m_bits[i] * ((self.q + 1) // 2)) % self.q for i in range(self.n)]
        ...
        for coeff in u_compressed:
            ct += coeff.to_bytes(4, 'little', signed=True)
        for coeff in v_compressed:
            ct += coeff.to_bytes(4, 'little', signed=True)
        ct += m

        # Derive shared secret
        ss = hashlib.sha3_256(ct[:-32] + m).digest()
    ...
    def decapsulate(self, sk: bytes, ct: bytes) -> bytes:
        ...
        for i in range(u_poly_size):
            u_compressed.append(int.from_bytes(ct[i*4:(i+1)*4], 'little', signed=True))
        v_compressed = []
        for i in range(self.n):
            v_compressed.append(int.from_bytes(ct[(u_poly_size + i)*4:(u_poly_size + i + 1)*4], 'little', signed=True))
        ...
        m_prime_bytes = ct[-32:]
        # Actually in `encapsulate`, m is directly added via `ct += m` which is 32 bytes
        ss = hashlib.sha3_256(ct[:-32] + m_prime_bytes).digest()
        return ss
...
class Dilithium3:
    ...
    def _unpack_public_key(self, pk: bytes) -> Tuple[bytes, List[List[int]]]:
        ...
        for i in range(self.k):
            poly = []
            for j in range(self.n):
                poly.append(int.from_bytes(t1_data[(i*self.n + j)*4:(i*self.n + j + 1)*4], 'little', signed=True))
            t1.append(poly)
        return rho, t1
    ...
    def sign(self, sk: bytes, msg: bytes) -> bytes:
        ...
        while True:
            ...
            w1_bytes = b''.join(c.to_bytes(4, 'little', signed=True) for poly in w1 for c in poly)
