...
impl Into<FileType> for MyFileType {
    fn into(self) -> FileType {
        match self {
            MyFileType::NamedPipe => FileType::NamedPipe,
            MyFileType::CharDevice => FileType::CharDevice,
            MyFileType::BlockDevice => FileType::BlockDevice,
            MyFileType::Directory => FileType::Directory,
            MyFileType::RegularFile => FileType::RegularFile,
            MyFileType::Symlink => FileType::Symlink,
            MyFileType::Socket => FileType::Socket,
        }
    }
}
...
impl CathedralFS {
    ...
    fn create_node(&mut self, parent: u64, name: &str, kind: MyFileType, theosis: f64) -> u64 {
        ...
        let node = CathedralNode {
            ...
            ctime: now,
            crtime: now,
            perm: if let MyFileType::Directory = kind { 0o755 } else { 0o644 },
            uid: 1000,
            gid: 1000,
            nlink: if let MyFileType::Directory = kind { 2 } else { 1 },
            theosis,
            merkle_hash: String::new(),
...
impl Filesystem for CathedralFS {
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        for node in self.nodes.values() {
            if node.parent == parent && node.name == name_str {
                ...
                node_to_reply = Some(node.clone());
                break;
            }
        }

        if let Some(node) = node_to_reply {
    ...
    fn mkdir(
        ...
        let ino = self.create_node(parent, &name_str, MyFileType::Directory, 0.7);
    ...
    fn mknod(
        ...
        let name_str = name.to_string_lossy().to_string();
        let ino = self.create_node(parent, &name_str, MyFileType::RegularFile, 0.5);
